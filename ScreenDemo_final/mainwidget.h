...
private slots:
    ...
    void refreshData()
    {
        //std::vector<int> array(100, 2048);
        numberHr->setText(QString::number(spr->HR1));
        numberSpo2->setText(QString::number(spr->SPO2_array1[0] / 10));
        widgetWaveEcg->waveWin->waveData = spr->ecg_array1;
        widgetWaveSpo2->waveWin->waveData = spr->SPO2_array1;
        widgetWaveResp->waveWin->waveData = spr->RESP_array1;
    }
...
public:
    ...
    {
        timer = new QTimer();
        connect(timer, QTimer::timeout, this, MainWidget::refreshData);
        timer->start(1000);
        ...
        QLabel * deviceNumberLabel = new QLabel();
        deviceNumberLabel->setText("  DEVICE: " + spr->device_number1 + "   " + "PATIENT: " + spr->patient_number1);
        ...
        pushButton->setStyleSheet("background-color:rgb(100,100,100);color:white;");
        ...
        layoutTitle->addWidget(pushButton);

        layoutTitle->setStretch(0, 1);
        layoutTitle->setStretch(1, 3);
        layoutTitle->setStretch(2, 5);
        layoutTitle->setStretch(3, 1);
